...
module.exports = class FastDependenciesAnalyzerPlugin {
  constructor(options = {}) {
    ...
    if (this.options.analyzeGitInfo.enable) {
      if (this.options.analyzeGitInfo.changeFileList.length) {
        ...
        this.options.analyzeGitInfo.commitRange = util.getCommitRange(this.options.analyzeGitInfo.commitRange);
      }
      this.options = util.merge(this.options, {
        tree: false,
        reverse: true,
  ...
  static async getGitCommitFileList(commitRange) {
    return util.getGitCommitFileList(util.getCommitRange(commitRange).join(' '));
  }
