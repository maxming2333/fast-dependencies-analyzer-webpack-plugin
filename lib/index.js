...
module.exports = class FastDependenciesAnalyzerPlugin {
  constructor(options = {}) {
    ...
    this.options = util.merge({
      ...
      analyzeGitInfo: {
        ...
        changeFileList: [],                                              // 修改的文件列表，如果不为空，那么就无视 commitRange 配置
  ...
  static async getGitCommitFileList(commitRange) {
    const range = await util.getCommitRange(commitRange);
    return util.getGitCommitFileList(range.join(' '));
  }
  ...
  async apply(compiler) {
    ...
    if (this.options.analyzeGitInfo.enable) {
      if (this.options.analyzeGitInfo.changeFileList.length) {
        this.options.analyzeGitInfo.commitRange = [];
      } else {
        this.options.analyzeGitInfo.commitRange = await util.getCommitRange(this.options.analyzeGitInfo.commitRange);
      }
      let fileList = this.options.analyzeGitInfo.changeFileList || [];
      if (!fileList.length) {
        fileList = await util.getGitCommitFileList(this.options.analyzeGitInfo.commitRange.join(' '));
      }
      const filterFileList = await this.options.analyzeGitInfo.fileFilter(fileList);
      this.gitCommitFileList = filterFileList.map(filePath => typeof filePath === 'string' ? filePath : filePath.filename);
    }
  }
};
