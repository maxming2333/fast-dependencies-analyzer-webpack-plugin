...
module.exports = class FastDependenciesAnalyzerPlugin {
  constructor(options = {}) {
    ...
    if (this.options.analyzeGitInfo.enable) {
      ...
      if (!commitRange.length) {
        const head = this.constructor.getGitHead();
  ...
  static skip(filePath) {
    return filePath && filePath.includes('node_modules');
  }

  static getCssDependencies(filePath) {
    let ret = [];
  ...
  static getGitCommitFileList(head) {
    return new Promise((resolve, reject) => {
  ...
  analyzeCss(filePath, ret = []) {
    const dependencies = this.constructor.getCssDependencies(filePath);
  ...
  afterResolve = (result, callback) => {
    ...
    if (issuer !== filePath && !this.constructor.skip(issuer) && !this.constructor.skip(filePath)) {
  ...
  async apply(compiler) {
    ...
    this.entryList = this.constructor.getEntryList(compiler.options).map(entry => this.conversionFilePath(entry));
    ...
    if (this.options.analyzeGitInfo.enable) {
      const fileList = await this.constructor.getGitCommitFileList(this.options.analyzeGitInfo.commitRange.join(' '));
